...
\begin{exerciselist}
\item[1.]\hypertarget{exercise-9}{}\hypertarget{p-138}{}%
...
\begin{enumerate}[label=(\alph*)]
\item\hypertarget{li-34}{}\hypertarget{p-139}{}%
What is the degree of vertex \(8\)?%
\item\hypertarget{li-35}{}\hypertarget{p-140}{}%
What is the degree of vertex \(10\)?%
\item\hypertarget{li-36}{}\hypertarget{p-141}{}%
How many vertices of degree \(2\) are there in \(\bfG\)? List them.%
\item\hypertarget{li-37}{}\hypertarget{p-142}{}%
Find a cycle of length \(8\) in \(\bfG\).%
\item\hypertarget{li-38}{}\hypertarget{p-143}{}%
What is the length of a shortest path from \(3\) to \(4\)?%
\item\hypertarget{li-39}{}\hypertarget{p-144}{}%
What is the length of a shortest path from \(8\) to \(7\)?%
\item\hypertarget{li-40}{}\hypertarget{p-145}{}%
...
\index{minimum weight spanning tree!Kruskal's algorithm for}\index{Kruskal's algorithm}\hypertarget{p-171}{}%
\leavevmode%
\begin{description}
...
\begin{example}[Kruskal's Algorithm]\label{example-9}
% group protects changes to lengths, releases boxes (?)
{% begin: group for a single side-by-side
% set panel max height to practical minimum, created in preamble
\setlength{\panelmax}{0pt}
...
\index{minimum weight spanning trees!Prim's algorithm for}\index{Prim's algorithm}\hypertarget{p-177}{}%
\leavevmode%
\begin{description}
...
\begin{example}[Prim's Algorithm]\label{example-10}
% group protects changes to lengths, releases boxes (?)
{% begin: group for a single side-by-side
% set panel max height to practical minimum, created in preamble
\setlength{\panelmax}{0pt}
...
\index{Dijkstra's algorithm}\index{graph!shortest path in}\hypertarget{p-191}{}%
...
\ifdefined\panelboxAp\else\newsavebox{\panelboxAp}\fi%
\savebox{\panelboxAp}{%
\raisebox{\depth}{\parbox{0.6\linewidth}{The lowest number that doesn't appear is 2, so we fill that in on the bottom of the first column. We no longer have to consider the 4 directly above this two, so we write in in parantheses.}}}
\ifdefined\phAp\else\newlength{\phAp}\fi%
\setlength{\phAp}{\ht\panelboxAp+\dp\panelboxAp}
\settototalheight{\phAp}{\usebox{\panelboxAp}}
\setlength{\panelmax}{\maxof{\panelmax}{\phAp}}
...
\ifdefined\panelboxAp\else\newsavebox{\panelboxAp}\fi%
\savebox{\panelboxAp}{%
\raisebox{\depth}{\parbox{0.6\linewidth}{To fill in the nextg cell, we put the lowest number not in parentheses, which is 3:}}}
\ifdefined\phAp\else\newlength{\phAp}\fi%
\setlength{\phAp}{\ht\panelboxAp+\dp\panelboxAp}
\settototalheight{\phAp}{\usebox{\panelboxAp}}
\setlength{\panelmax}{\maxof{\panelmax}{\phAp}}
...
\ifdefined\panelboxAp\else\newsavebox{\panelboxAp}\fi%
\savebox{\panelboxAp}{%
\raisebox{\depth}{\parbox{0.6\linewidth}{And now the lowest term not in parentheses is 6, so we add that:}}}
\ifdefined\phAp\else\newlength{\phAp}\fi%
\setlength{\phAp}{\ht\panelboxAp+\dp\panelboxAp}
\settototalheight{\phAp}{\usebox{\panelboxAp}}
\setlength{\panelmax}{\maxof{\panelmax}{\phAp}}
...
\ifdefined\panelboxAp\else\newsavebox{\panelboxAp}\fi%
\savebox{\panelboxAp}{%
\raisebox{\depth}{\parbox{0.6\linewidth}{Now the only 1 appearing is in parenthesis, so we add that in the next column:}}}
\ifdefined\phAp\else\newlength{\phAp}\fi%
\setlength{\phAp}{\ht\panelboxAp+\dp\panelboxAp}
\settototalheight{\phAp}{\usebox{\panelboxAp}}
\setlength{\panelmax}{\maxof{\panelmax}{\phAp}}
...
\ifdefined\panelboxAp\else\newsavebox{\panelboxAp}\fi%
\savebox{\panelboxAp}{%
\raisebox{\depth}{\parbox{0.6\linewidth}{Having constructed the table encoding all the edges, we can now draw the labelled tree with thsoe edges}}}
\ifdefined\phAp\else\newlength{\phAp}\fi%
\setlength{\phAp}{\ht\panelboxAp+\dp\panelboxAp}
\settototalheight{\phAp}{\usebox{\panelboxAp}}
\setlength{\panelmax}{\maxof{\panelmax}{\phAp}}
\ifdefined\panelboxAlatex-image\else\newsavebox{\panelboxAlatex-image}\fi%
\savebox{\panelboxAlatex-image}{%
\parbox{70pt}{[latex-image]}}
\ifdefined\phAlatex-image\else\newlength{\phAlatex-image}\fi%
\setlength{\phAlatex-image}{\ht\panelboxAlatex-image+\dp\panelboxAlatex-image}
\settototalheight{\phAlatex-image}{\usebox{\panelboxAlatex-image}}
\setlength{\panelmax}{\maxof{\panelmax}{\phAlatex-image}}
\leavevmode%
% begin: side-by-side as tabular
% \tabcolsep change local to group
\setlength{\tabcolsep}{0.025\linewidth}
...
\begin{tabular}{@{}*{2}{c}@{}}
\begin{minipage}[c][\panelmax][c]{0.6\linewidth}\usebox{\panelboxAp}\end{minipage}&
\begin{minipage}[c][\panelmax][c]{0.3\linewidth}\usebox{\panelboxAlatex-image}\end{minipage}\end{tabular}\\
% end: side-by-side as tabular
}% end: group for a single side-by-side
\end{example}
\typeout{************************************************}
